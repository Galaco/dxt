...
func decompressDxt1(packed []byte, width int, height int) ([]color.RGBA,error) {
	...
	for j := 0; j < blockCountY; j++ {
		for i := 0; i < blockCountX; i++ {
			decompressDxt1Block(packed[offset+(i * 8):], i*4, j*4, width, unpacked)
		}
...
func decompressDxt1Block(packed []byte, offsetX int, offsetY int, width int, unpacked []color.RGBA) {
	// Construct colours to transform between
	var c0, c1 uint16
	binary.Read(bytes.NewBuffer(packed[:2]), binary.LittleEndian, &c0)
	binary.Read(bytes.NewBuffer(packed[2:4]), binary.LittleEndian, &c1)

	colour0 := common.Rgb565toargb8888(c0)
	colour1 := common.Rgb565toargb8888(c1)

	var code uint32
	binary.Read(bytes.NewBuffer(packed[4:]), binary.LittleEndian, &code)
